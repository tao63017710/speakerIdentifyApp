...
public class WaveCollectActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wave_collect);

        startButton = findViewById(R.id.waveCollectStartBtn);
        stopButton = findViewById(R.id.waveCollectStopBtn);
    ...
    private void startRecording() {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    //创建文件夹
                    String origin_dirPath = getFilesDir().getAbsolutePath() + "/voice_features" + "/wave_original/" + personId;
                    String numpy_path = getFilesDir().getAbsolutePath() + "/voice_features" + "/wave_numpy/" + personId;
                    File destDir = new File(origin_dirPath);
                    if (!destDir.exists()) {
                        destDir.mkdirs();
                    }
                    destDir = new File(numpy_path);
                    if (!destDir.exists()) {
                        destDir.mkdirs();
                    }

                    long nameTime = System.currentTimeMillis();
                    String dataPath = origin_dirPath + "/" + nameTime + ".wav";
                    String numpyPath = numpy_path + "/" + nameTime + ".npy";
                    ...
                    //提取特征
                    initPython();
                    boolean registerCheck = registerSpeaker(dataPath, numpyPath);
                    //音频没问题，插入数据库
                    if(registerCheck){
                        try {
                            DatabaseHelper helper = new DatabaseHelper(WaveCollectActivity.this, "feature_identify.db", null, 1);
                    ...
                    if (errorText != null) {
                        Looper.prepare();
                        Toast.makeText(WaveCollectActivity.this, errorText, Toast.LENGTH_SHORT).show();
                        Looper.loop();
                    }
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    private boolean registerSpeaker(String filePath, String numpyAddr){
        Python python = Python.getInstance();
        PyObject pyCheck = python.getModule("register_android_edition1").callAttr("register",filePath, numpyAddr);
        boolean check = pyCheck.toBoolean();
